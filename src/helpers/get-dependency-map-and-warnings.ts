...
export const getDependencyMapAndWarnings = (
  ...
  tasks.forEach(task => {
    ...
    const dependenciesByLevel =
      dependencyRes.get(comparisonLevel) || new Map<string, ExpandedDependency[]>();
    const dependentsByLevel =
      dependentRes.get(comparisonLevel) || new Map<string, ExpandedDependent[]>();
    ...
    dependencies.forEach(({ sourceId, sourceTarget, ownTarget }) => {
      ...
      const { y: fromY } = getCoordinatesOnLevel(sourceId, coordinatesOnLevelMap);
      ...
      if (isCollectMargins) {
        const taskTime = ownTarget === 'startOfTask' ? task.start.getTime() : task.end.getTime();

        const sourceTime =
          sourceTarget === 'startOfTask' ? source.start.getTime() : source.end.getTime();
