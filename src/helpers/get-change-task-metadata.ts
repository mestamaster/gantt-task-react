...
const collectSuggestedParents = (changeAction: ChangeAction, tasksMap: TaskMapByLevel) => {
...
const getSuggestedStartEndChangesFromDirectChildren = (
  ...
  const tasks = Array.from(id2Task.values()).filter(({ type }) => type !== 'empty') as Task[];
  const directChildren = tasks
    .filter(task => {
      // as the task is deleted, it must be ignored in the parent start end
      if (changeAction.type == 'delete' && changeAction.tasks.map(t => t.id).includes(task.id))
        return false;
