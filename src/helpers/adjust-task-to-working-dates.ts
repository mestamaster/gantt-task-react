...
import { ONE_DAY_DURATION, ONE_HOUR_DURATION, getStepTime } from './round-task-dates';
...
export const adjustTaskToWorkingDates = ({
  ...
  switch (changedTask.type) {
    case 'milestone':
      if (checkIsHoliday(new Date(changedTask.start.getTime() + 1000 * 3600), 'startOfTask')) {
        const nextWorkingDate = getNextWorkingDate(changedTask.start, action, 'startOfTask');
      ...
      switch (action) {
        case 'end': {
          if (checkIsHoliday(changedTask.end, 'endOfTask')) {
            const moveEndDelta = changedTask.end.getTime() - originalTask.end.getTime();
            const endDate =
              moveEndDelta < 0
        ...
        case 'start': {
          if (checkIsHoliday(new Date(changedTask.start.getTime() + 1000 * 3600), 'startOfTask')) {
            const moveStartDelta = changedTask.start.getTime() - originalTask.start.getTime();
            return {
              ...changedTask,
              start:
                moveStartDelta > 0
                  ? getNextWorkingDate(changedTask.start, action, 'startOfTask')
                  : getPreviousWorkingDate(changedTask.start, action, 'startOfTask'),
            };
          }
        ...
        case 'move': {
          ...
          const moveDelta = changedTask.start.getTime() - originalTask.start.getTime();
          if (moveDelta !== 0) {
            ...
            const stepTime = Math.max(getStepTime(dateMoveStep), ONE_HOUR_DURATION);

            const movingToTheRight = moveDelta >= 0;
            if (movingToTheRight) {
              if (checkIsHoliday(changedTask.start, 'startOfTask')) {
                newStart = getNextWorkingDate(changedTask.start, action, 'startOfTask');
              }
              ...
              let newDurationWithoutHolidays = getDuration(newStart, newEnd, checkIsHoliday);
              ...
              while (
                ...
                newDurationWithoutHolidays = getDuration(newStart, newEnd, checkIsHoliday);
              }
            } else {
              if (checkIsHoliday(changedTask.end, 'endOfTask')) {
                newEnd = getPreviousWorkingDate(changedTask.end, action, 'endOfTask');
              }

              newStart = new Date(newEnd.getTime() - durationWithoutHolidays);
              if (checkIsHoliday(newStart, 'startOfTask')) {
                newStart = getPreviousWorkingDate(newStart, action, 'startOfTask');
              }
              let newDurationWithoutHolidays = getDuration(newStart, newEnd, checkIsHoliday);
...
const getDuration = (
  ...
  const newDurationWithoutHolidays =
    endDate.getTime() - startDate.getTime() - newHolidaysLength * ONE_DAY_DURATION;
